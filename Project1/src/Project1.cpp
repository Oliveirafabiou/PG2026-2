	...
	*/
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (elapsedSeconds > 0.05) {
			...
			matrix[12] = elapsedSeconds * speed * cos(rad) + posX;
			matrix[13] = elapsedSeconds * speed * sin(rad) + posY;

			posX = matrix[12];
			posY = matrix[13];

			glm::vec2 direction = { cos(rad), sin(rad) };

			// test collision - top
			if (posY + points[0].coords[1] > 1.0f || posY + points[1].coords[1] > 1.0f || posY + points[2].coords[1] > 1.0f) {
				glm::vec2 borderNormal = { 0.0f, 1.0f };
				glm::vec2 rOut = { 2 * borderNormal.x * borderNormal.x * (-direction.x) + direction.x,
								   2 * borderNormal.y * borderNormal.y * (-direction.y) + direction.y };
			...
			if (posX + points[0].coords[0] > 1.0f || posX + points[1].coords[0] > 1.0f || posX + points[2].coords[0] > 1.0f) {
				glm::vec2 borderNormal = { 1.0f, 0.0f };
				glm::vec2 rOut = { 2 * borderNormal.x * borderNormal.x * (-direction.x) + direction.x,
								   2 * borderNormal.y * borderNormal.y * (-direction.y) + direction.y };
			...
			deg = atan2(direction.y, direction.x) * 180 / M_PI;
		}
